...
function App() {
  ...
  const handTranslate = async () => {
    ...
    try {
      ...
      console.log(data); // Adicionando um log para verificar a resposta
      setTranslatedText(data.responseData.translatedText);
    } catch (erro) {
      setError(`Erro ao tentar traduzir: ${erro.message}. Tente novamente.`);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className='min-h-screen bg-background flex flex-col'>
      ...
      <main className='flex-grow flex items-start justify-center px-4 py-8'>
        <div className='w-full max-w-5xl bg-white rounded-lg shadow-md overflow-hidden'>
          ...
          {error && (
            <div className='p-4 bg-red-500 border-t border-red-200 text-red-800'>
              {error}
            </div>
          )}
        </div>
      ...
      <footer className='bg-white border-t border-gray-300 mt-auto'>
        <div className='max-w-5xl mx-auto px-4 py-3 text-sm text-headerColor font-bold'>
          &copy; {new Date().getFullYear()} Tradutor LK
        </div>
      </footer>
    </div>
  );
